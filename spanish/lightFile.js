...
function createLMap(){
   ...
   for (i=0; i<lights.length;i++){
    if(lights[i].Exists == 'No'){
       ...
       data.push({location: new google.maps.LatLng(lights[i].Location.Latitude,lights[i].Location.Longitude) ,weight: 1.5});
    else if(lights[i].Type == 'Double Lamp')
       data.push({location: new google.maps.LatLng(lights[i].Location.Latitude,lights[i].Location.Longitude) , weight: 3.5});
    else if(lights[i].Type == 'Parking Lot')
